/**
 * ServerActivityChart — real-time line chart tracking total visits per game.
 */
...
export default function ServerActivityChart({ games, visitHistory }) {
  ...
  useEffect(() => {
    if (!initialized.current && games.length > 0) {
      ...
      const top = games.reduce((best, g) => g.visits > best.visits ? g : best, games[0]);
      setHidden(new Set(games.filter(g => g.universeId !== top.universeId).map(g => g.universeId)));
    }
  }, [games]);

  const totalVisits = games.reduce((s, g) => s + g.visits, 0);
  const latestTime =
    visitHistory.length > 0
      ? visitHistory[visitHistory.length - 1].time
      : 0;
  const earliestTime =
    visitHistory.length > 0 ? visitHistory[0].time : 0;
  ...
  const eligibleGames = useMemo(() => {
    ...
    const pts = visitHistory.filter((h) => h.time >= cutoff);
    const fallback = [{ time: now, visits: Object.fromEntries(games.map((g) => [g.universeId, g.visits])) }];
    const points = pts.length > 0 ? pts : fallback;
    return games.filter((g) =>
      points.some((h) => (h.visits[g.universeId] || 0) > 0)
    );
  }, [games, visitHistory, range, latestTime]);
  ...
  const { chartData, chartOptions } = useMemo(() => {
    ...
    const filtered = visitHistory.filter((h) => h.time >= rangeCutoff);

    const points =
      filtered.length > 0
        ? filtered
        : [{ time: now, visits: Object.fromEntries(games.map((g) => [g.universeId, g.visits])) }];
    ...
    for (const g of shownGames) {
      for (const h of points) {
        const v = h.visits[g.universeId] || 0;
        if (v < yMin) yMin = v;
        if (v > yMax) yMax = v;
      }
    }
      ...
      chartData: {
        datasets: shownGames.map((g, i) => {
          ...
          return {
            ...
            data: points.map((h) => ({ x: h.time, y: h.visits[g.universeId] || 0 })),
        ...
        plugins: {
          ...
          tooltip: {
            ...
            callbacks: {
              ...
              label: (ctx) =>
                ` ${ctx.dataset.label}: ${formatNumber(ctx.parsed.y)} visits`,
            },
          },
        },
        scales: {
  ...
  }, [games, visitHistory, range, latestTime, hidden, eligibleGames, dataSpanMs, earliestTime]);
      ...
      <div className="chart-head">
        <div className="chart-head-left">
          ...
          <span className="chart-stat">
            {formatNumber(totalVisits)} all-time
          </span>
        </div>
